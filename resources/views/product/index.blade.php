...
<div class="w-full pt-10 px-4 sm:px-6 md:px-8 lg:pl-72">
    ...
    <div>
        <div class="max-w-screen-lg px-4 py-8 sm:px-8">
            <div class="overflow-y-hidden border">
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="text-gray-500">
                            ...
                            <tr>
                                ...
                                <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                    <p class="whitespace-no-wrap">{{ $item->jenis->name }}</p>
                                </td>
                                <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                    <p class="whitespace-no-wrap">{{ number_format($item->harga,0,',','.') }}</p>
                                </td>
                                <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                    <img class="rounded-lg w-20 h-20"
                                        src="{{ asset(str_replace('public','storage',$item->foto)) }}"
                                        alt="Image Description">
                                </td>
                                <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                    <a href="{{ route('product.edit',$item->id) }}"
                                        class="text-hijau hover:underline px-2">Edit</a>
                                    <form action="{{ route('product.destroy',$item->id) }}" method="post"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:underline px-2">Delete</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
